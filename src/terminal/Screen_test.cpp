...
namespace // {{{
{
    string mainPageText(Screen const& screen)
    {
...
// Text spans 3 lines.
TEST_CASE("writeText.bulk.F", "[screen]") // TODO(pr)
{
    auto screen = MockScreen{PageSize{LineCount(3), ColumnCount(10)},
                             LineCount(1)};
    screen.write("a");
...
{
    auto screen = MockScreen{PageSize{LineCount(2), ColumnCount(10)},
                             LineCount(1)};
...
// TODO(pr) Test spanning writes over all history and then reusing old lines.
...
{
    auto screen = MockScreen{PageSize{LineCount(1), ColumnCount(4)}};
    ...
    screen.write(U"\uFE0E");
    REQUIRE(*screen.logicalCursorPosition().column == 2); // U+FE0E does *NOT* lower width to 1 (easier to implement)
    screen.write(U"X");
...
{
    auto screen = MockScreen{PageSize{LineCount(1), ColumnCount(2)}, LineCount(5)};
...
{
    auto screen = MockScreen{PageSize{LineCount(5), ColumnCount(5)}, LineCount(1)};
    screen.write("12345\r\n67890\r\nABCDE\r\nFGHIJ\r\nKLMNO\r\nPQRST\033[H");
    REQUIRE("67890\nABCDE\nFGHIJ\nKLMNO\nPQRST\n" == screen.renderMainPageText());
    REQUIRE(screen.logicalCursorPosition() == Coordinate{LineOffset(0), ColumnOffset(0)});
...
{
    auto screen = MockScreen{PageSize{LineCount(2), ColumnCount(3)}, LineCount{5}};
    screen.write("123\r\n456\r\nABC\r\nDEF");
...
{
    auto screen = MockScreen{PageSize{LineCount(2), ColumnCount(5)}, LineCount{5}};
    ...
    SECTION("lines: 0") {
        ...
        INFO(e(screen.replyData));
        CHECK(e(screen.replyData) == e("\033]314;\033\\"));
    }
    SECTION("lines: 1") {
        screen.captureBuffer(1, false);
        INFO(e(screen.replyData));
        CHECK(e(screen.replyData) == e("\033]314;KLMNO\n\033\\\033]314;\033\\"));
    }
    SECTION("lines: 2") {
        screen.captureBuffer(2, false);
        INFO(e(screen.replyData));
        CHECK(e(screen.replyData) == e("\033]314;FGHIJ\nKLMNO\n\033\\\033]314;\033\\"));
    }
    SECTION("lines: 3") {
        screen.captureBuffer(3, false);
        INFO(e(screen.replyData));
        CHECK(e(screen.replyData) == e("\033]314;ABCDE\nFGHIJ\nKLMNO\n\033\\\033]314;\033\\"));
    }
    SECTION("lines: 4") {
        screen.captureBuffer(4, false);
        INFO(e(screen.replyData));
        CHECK(e(screen.replyData) == e("\033]314;67890\nABCDE\nFGHIJ\nKLMNO\n\033\\\033]314;\033\\"));
    }
    SECTION("lines: 5") {
        screen.captureBuffer(5, false);
        INFO(e(screen.replyData));
        CHECK(e(screen.replyData) == e("\033]314;12345\n67890\nABCDE\nFGHIJ\nKLMNO\n\033\\\033]314;\033\\"));
    }
...
TEST_CASE("render into history", "[screen]")
{
    ...
    string renderedText;
    renderedText.resize((screen.pageSize().columns + 1).as<size_t>()
                       * screen.pageSize().lines.as<size_t>());

    auto const renderer = [&](Cell const& cell, LineOffset _row, ColumnOffset _column) {
        auto const offset = _row.as<size_t>() * (screen.pageSize().columns + 1).as<size_t>()
                          + _column.as<size_t>();
        renderedText.at(offset) = cell.codepointCount()
                                ? static_cast<char>(cell.codepoint(0))
                                : ' ';
...
{
    auto screen = MockScreen{PageSize{LineCount(3), ColumnCount(4)}, LineCount(10)};
    REQUIRE_FALSE(screen.findMarkerDownwards(ScrollOffset(0)).has_value());
    REQUIRE_FALSE(screen.findMarkerDownwards(ScrollOffset(1)).has_value()); // history bottom
    REQUIRE_FALSE(screen.findMarkerDownwards(ScrollOffset(2)).has_value());
    REQUIRE_FALSE(screen.findMarkerDownwards(ScrollOffset(3)).has_value()); // history top
    REQUIRE_FALSE(screen.findMarkerDownwards(ScrollOffset(4)).has_value()); // overflow
    ...
    SECTION("no marks") {
        ...
        auto mark = screen.findMarkerDownwards(ScrollOffset(4));
        REQUIRE_FALSE(mark.has_value());

        // scroll-top
        mark = screen.findMarkerDownwards(ScrollOffset(3));
        REQUIRE_FALSE(mark.has_value());

        mark = screen.findMarkerDownwards(ScrollOffset(2));
        REQUIRE_FALSE(mark.has_value());

        mark = screen.findMarkerDownwards(ScrollOffset(1));
        REQUIRE_FALSE(mark.has_value());

        // underflow: one below scroll buttom
        mark = screen.findMarkerDownwards(ScrollOffset(0));
        REQUIRE_FALSE(mark.has_value());
    ...
    SECTION("with marks") {
        ...
        auto marker = screen.findMarkerDownwards(ScrollOffset(4));
        REQUIRE(marker.has_value());
        REQUIRE(*marker.value() == 1);

        // scroll top -> scroll bottom
        marker = screen.findMarkerDownwards(ScrollOffset(3));
        REQUIRE(marker.has_value());
        REQUIRE(*marker.value() == 1);

        // scroll bottom -> NONE
        marker = screen.findMarkerDownwards(ScrollOffset(1));
        REQUIRE(marker.has_value());
...
{
    auto screen = MockScreen{PageSize{LineCount(3), ColumnCount(4)}, LineCount(10)};
    REQUIRE_FALSE(screen.findMarkerUpwards(ScrollOffset(-1)).has_value()); // peak into history
    REQUIRE_FALSE(screen.findMarkerUpwards(ScrollOffset(0)).has_value());
    REQUIRE_FALSE(screen.findMarkerUpwards(ScrollOffset(1)).has_value());
    REQUIRE_FALSE(screen.findMarkerUpwards(ScrollOffset(2)).has_value());
    REQUIRE_FALSE(screen.findMarkerUpwards(ScrollOffset(3)).has_value()); // overflow
    ...
    SECTION("no marks") {
        ...
        auto mark = screen.findMarkerUpwards(ScrollOffset(0));
        REQUIRE_FALSE(mark.has_value());

        // bottom line in history
        mark = screen.findMarkerUpwards(ScrollOffset(1));
        REQUIRE_FALSE(mark.has_value());

        // one above bottom line in history
        mark = screen.findMarkerUpwards(ScrollOffset(2));
        REQUIRE_FALSE(mark.has_value());

        // top history line
        mark = screen.findMarkerUpwards(ScrollOffset(3));
        REQUIRE_FALSE(mark.has_value());

        // one above history top
        mark = screen.findMarkerUpwards(ScrollOffset(4));
        REQUIRE_FALSE(mark.has_value());
    }
    ...
    SECTION("with marks") {
        ...
        auto marker = screen.findMarkerUpwards(ScrollOffset(0));
        REQUIRE(marker.has_value());
        REQUIRE(marker.value().value == 1); // 3ghi

        // scroll offset 1 -> scroll offset 3
        marker = screen.findMarkerUpwards(ScrollOffset(1));
        REQUIRE(marker.has_value());
        REQUIRE(marker.value().value == 3); // 1abc

        // one-off
        marker = screen.findMarkerUpwards(ScrollOffset(3));
        REQUIRE(!marker.has_value());
    }
}
...
TEST_CASE("DECTABSR", "[screen]")
{
    ...
    SECTION("default tabstops") {
        ...
        CHECK(e(screen.replyData) == e("\033P2$u1/9/17/25/33\033\\"));
    }
    ...
    SECTION("custom tabstops") {
        ...
        screen.requestTabStops();
        CHECK(e(screen.replyData) == e("\033P2$u2/4/8/16\033\\"));
    }
}
...
TEST_CASE("OSC.2.Unicode")
{
    ...
    CHECK(e(screen.windowTitle) == e(title));
}
...
TEST_CASE("DECCRA.Right.intersecting", "[screen]")
{
    ...
    auto const deccraSeq = fmt::format("\033[{};{};{};{};{};{};{};{}$v",
            sTopLeft.line + 1, sTopLeft.column + 1,
            sBottomRight.line + 1, sBottomRight.column + 1, page,
            tTopLeft.line + 1, tTopLeft.column + 1, page);
    screen.write(deccraSeq);
