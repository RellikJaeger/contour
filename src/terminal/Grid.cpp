...
namespace detail { // {{{
    ...
    LineCount addNewWrappedLines(
        ...
        bool _initialNoWrap // TODO(pr): pass `LineFlags _defaultLineFlags` instead?
    )
    {
...
LineCount Grid<Cell>::scrollUp(LineCount _n, GraphicsAttributes _defaultAttributes, Margin _margin) noexcept
{
    ...
    Expects(*_margin.vertical.from >= 0 && *_margin.vertical.to < *pageSize_.lines);
...
Coordinate Grid<Cell>::resize(PageSize _newSize, Coordinate _currentCursorPos, bool _wrapPending)
{
    ...
    auto const shrinkLines = [this](LineCount _newHeight, Coordinate _cursor) -> Coordinate
    {
        ...
        if (*_cursor.line + 1 < *pageSize_.lines)
        {
            ...
            auto const linesAvailableBelowCursor = pageSize_.lines - boxed_cast<LineCount>(_cursor.line + 1);
            auto const cutoffCount = min(numLinesToShrink, linesAvailableBelowCursor);
            pageSize_.lines -= cutoffCount;
            linesUsed_ -= cutoffCount;
            Ensures(*_cursor.line + 1 <= *pageSize_.lines);
            verifyState();
        }

        // 2.) If _newHeight is still below page line count, then shrink by rotating up.
        auto const numLinesToShrink = min(maxHistoryLineCount_, pageSize_.lines - _newHeight);
        if (*numLinesToShrink > 0)
        {
            Expects(*_cursor.line + 1 == *pageSize_.lines);
            rotateBuffersLeft(numLinesToShrink);
            pageSize_.lines -= numLinesToShrink;
            clampHistory();
            verifyState();
            Ensures(_newHeight == pageSize_.lines);
            return Coordinate{-boxed_cast<LineOffset>(numLinesToShrink), {}};
        }

        // linesUsed_ -= pageSize_.lines - _newHeight;
        // pageSize_.lines = _newHeight;
        // clampHistory();
        // verifyState();
        return Coordinate{};
    };
    ...
    auto const shrinkColumns = [this](ColumnCount _newColumnCount, LineCount _newLineCount, Coordinate _cursor) -> Coordinate
    {
        ...
        else
        {
            ...
            while (shrinkedLines.size() < totalLineCount)
                shrinkedLines.emplace_back(_newColumnCount, defaultLineFlags());
